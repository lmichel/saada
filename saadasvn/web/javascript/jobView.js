jQuery.extend({
	...
	JobView: function(){
		...
		this.initForm = function(id, phase, actions){
			$('#' + that.containerID).prepend("<div id=" + id + " style='float: none;'></div>");
			$('#' + id).html('');
			$('#' + id).append('<span id=' + id + '_id>Job "' + id + '"</span>');
			$('#' + id).append('&nbsp;<span id=' + id + '_phase>' + phase + '</span>');
			$('#' + id).append('<select id=' + id + '_actions style="font-size: small;" onChange="tapView.fireJobAction(\'' + id + '\');"></select>');
