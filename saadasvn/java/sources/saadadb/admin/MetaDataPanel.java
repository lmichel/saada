...
import saadadb.admin.popup.PopupBaseNode;
import saadadb.admin.popup.PopupCatNode;
import saadadb.admin.popup.PopupClassNode;
import saadadb.admin.popup.PopupCollNode;
...
public class MetaDataPanel extends JPanel implements DragGestureListener,  DragSourceListener {
	...
	public MetaDataPanel(Frame frame, int largeur, int hauteur) throws SaadaException {
		this.frame = frame;
		...
		tree.addMouseListener(new MouseAdapter() {
			...
			public void mousePressed(MouseEvent e) {
				...
				if( path != null && (path.getPathCount() == 4 || path.getPathCount() == 3) && e.getClickCount() == 2 ) {
					try {
						if( MetaDataPanel.this.frame instanceof SaadaDBAdmin)
							((SaadaDBAdmin)(MetaDataPanel.this.frame)).showProduct(path.getPath());
