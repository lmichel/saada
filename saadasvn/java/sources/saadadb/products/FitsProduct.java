...
public class FitsProduct extends File implements ProductFile{
	 ...
	 */
	public LinkedHashMap<String, Integer> getTableEntry(){
...
//				}
	...
	public Image getBitMapImage() throws FitsException, IOException{
		ImageHDU himage = (ImageHDU)this.fits_data.getHDU(this.product.configuration.getHeaderRef().getNumber());
	...
	public FitsProduct(Product product) throws Exception {
		...
		this.product = product;
		this.fits_data = new Fits(product.dataPointer.localFileName);
		try {
		...
		if( product.getConfiguration() != null ) {
			ext_name = product.getConfiguration().getExtensionName();
		}
		if( ext_name != null && !ext_name.equals("") ){	//if it is defined
			 ...
			 */
				...
				ext_num = getHeaderNumber(product.getConfiguration().getExtensionName());
			}
			/*
			...
			if( ext_num >= 0  && (bHDU = fits_data.getHDU(ext_num)) != null ) {
				if( this.checkExtensionCategory(bHDU, this.getFITSExtensionCategory() )) {
					...
					Messenger.printMsg(Messenger.TRACE, "Required extension : "+product.getConfiguration().getExtensionName()+" found (number: " + this.good_header_number + ")" );
				}
				/*
				 * If no BINTABLE has been found for spectra, we try to find out a one-row image
				 */
				else if( product.getConfiguration().getCategorySaada() == Category.SPECTRUM ) {
					if( (bHDU = fits_data.getHDU(ext_num))  != null &&  checkExtensionCategory(bHDU, "IMAGE") ) {
						...
						IgnoreException.throwNewException(SaadaException.WRONG_RESOURCE, "Required extension : "+product.getConfiguration().getExtensionName() + " has a wrong type: " + bHDU.getClass().getName());
					}

				}
				else {
					IgnoreException.throwNewException(SaadaException.WRONG_RESOURCE, "Required extension : "+product.getConfiguration().getExtensionName() + " has a wrong type");
				}
			}
			else {
				IgnoreException.throwNewException(SaadaException.WRONG_RESOURCE, "Required extension : "+product.getConfiguration().getExtensionName() + " not found");
			}
		}
