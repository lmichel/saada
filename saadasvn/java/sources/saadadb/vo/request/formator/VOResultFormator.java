 ...
 * @version 07/2011
 */
public abstract class VOResultFormator implements Formator{
	...
	public void setProtocolParams(Map<String, String> fmtParams) throws Exception{
		...
		if( (rel = fmtParams.get("relations")) != null ) {
			...
			if( all ) {
				for( String dr: drls ) {
					Messenger.printMsg(Messenger.TRACE, "Add relation " + dr + " in response");
			...
			// Match existing relations with requested relations
				...
				for( String r: rls ) {
					...
					if( !found ) {
						...
						Messenger.printMsg(Messenger.TRACE, "Add relation " + r + " in response");
