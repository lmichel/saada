...
public class FormPanel extends JPanel {
	...
	, new JRadioButton("Energy")
	, new JRadioButton("Frequency")
	, new JRadioButton("Wavelength")};
	protected String[][] sp_units = { new String[0]
	                                             , {"eV" ,"keV" ,"MeV","GeV","TeV"} 
	, {"Hz", "kHz", "MHz", "GHz"}
	, {"Angstrom", "nm", "um", "m", "mm" , "cm", "km", "nm"}
	};
	...
	public void jumpToPanel(int screen_number)  {
		...
		break;
		case 3: this.setWebRootPanel(); break;
		case 4: this.setNewAttPanel(); break;
		case 5: this.setSysAndUnitPanel(); break;
		}
	}
	...
	public boolean validPanel(int screen_number) {
		switch( screen_number ) {
		case 0: return this.validSaadaDirPanel(); 
		case 1: return this.validDatabaseConnectPanel(); 
		case 2: return this.validDatabaseReaderPanel();
		case 3: return this.validWebRootPanel(); 
		case 4: return this.validNewAttPanel(); 
		case 5: return this.validSysAndUnitPanel(); 
		}
		return false;
	}
	...
	public void setSaadaDirPanel() {
		...
		c0.gridx = 0 ; c0.gridy = 2 ; this.add(jp, c0);
	...
	protected boolean validSaadaDirPanel() {
		...
		if( saadadb_rep.getText().trim().equals(saadadb_home.getText().trim()) ) {
			JOptionPane.showMessageDialog(frame,
					"The SaadaDB installation can not be the same as the repository",
					"Configuration Error",
					JOptionPane.ERROR_MESSAGE);
			return false;
		}
	...
	public void setDatabaseConnectPanel() {
		...
		c.gridx = 1; c.gridy = 0;	
		c.fill  = GridBagConstraints.HORIZONTAL;
		//		dbms_combo.add(new JLabel("SQLite"));
		//		dbms_combo.add(new JLabel("Postgres"));
		//		dbms_combo.add(new JLabel("MySQL"));
	...
	private void enableDatabaseConnectPanel(boolean enabled) {
		for( Component cp: dbms_panel.getComponents() ) {
			if( !(cp instanceof JComboBox) && cp.toString().indexOf("Manage") == -1)
		...
		try {
			...
			if( dbmswrapper.dbExists(rep_path, db_local_name ) ) {
				if( JOptionPane.showConfirmDialog(frame,
						"Relational database <" + db_local_name + "> already exists. Do you want to remove it?",
			...
			Messenger.printStackTrace(e);
			JOptionPane.showMessageDialog(frame,
					"Fatal error: Can not build the DB wrapper: " + e,
					"Configuration Error",
					JOptionPane.ERROR_MESSAGE);
			retour = false;
	...
	protected boolean validDatabaseReaderPanel() {
		...
		try {
			if( dbmswrapper == null ) {
				JOptionPane.showMessageDialog(frame,
						"Fatal error: DBMS wrapper not set: valid the prvious screen please.",
						"Configuration Error",
						JOptionPane.ERROR_MESSAGE);
				return false;
			}
			...
			Messenger.printStackTrace(e);
			JOptionPane.showMessageDialog(frame,
					"Internal Fatal error: " + e.getMessage(),
					"Configuration Error",
					JOptionPane.ERROR_MESSAGE);
			retour = false;
			...
			Messenger.printStackTrace(e);
			JOptionPane.showMessageDialog(frame,
					"Thread Fatal error: " + e.getMessage(),
					"Configuration Error",
					JOptionPane.ERROR_MESSAGE);
			retour = false;
		} catch (Exception e) {
	...
	/*
	 * In charge of populating the combo with attribute made for the selected category
	 */
		...
		button.addActionListener(new ActionListener() {
			public void actionPerformed(@SuppressWarnings("unused") ActionEvent arg0) {
				...
				if( name.matches(RegExp.EXTATTRIBUTE) ) {
					...
					JOptionPane.showMessageDialog(frame,
							"Attribute name <" + name + "> badly formed",
							"Configuration Error",
							JOptionPane.ERROR_MESSAGE);					
				}
			}
	...
	protected boolean validNewAttPanel() {
		try{
			...
			JOptionPane.showMessageDialog(frame,
					"Writing extended attr file: " + ex.getMessage(),
					"Configuration Error",
					JOptionPane.ERROR_MESSAGE);
			return false;			
		}
	...
	/*
	 * In charge of populating the combo with spectral units
	 */
	...
	protected boolean validSysAndUnitPanel() {
		...
		try {
			...
			if( dbmswrapper == null ) {
				JOptionPane.showMessageDialog(frame,
						"DBMS wrapper not set, please fill all panels",
						"Configuration Error",
						JOptionPane.ERROR_MESSAGE);
				return false;							
			}
			...
			Messenger.printStackTrace(e);
			JOptionPane.showMessageDialog(frame,
					"Writing saadadb confif file: " + e.getMessage(),
					"Configuration Error",
					JOptionPane.ERROR_MESSAGE);
			return false;			
		}
