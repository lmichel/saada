...
public class Unit {
	...
	static final private long _NOUNIT    = 0x0230303030303030L;	// Unitless
	...
	public final void mag() throws ArithmeticException {
		...
		if ((mksa == _NOUNIT) && (factor == 1)) {	// Now we've mag
			mksa |= _mag;
	 ...
	 * @throws ArithmeticException
	 */
	public static String convertStr(Unit source_unit, Unit target_unit) 
	throws ArithmeticException {
		String s = source_unit.symbol + Converter.SEP + target_unit.symbol;
		Object o = hConv.get(s);
		if (o != null) {
			Converter c = (Converter)o;
			return c.convertStr(source_unit.value);
		}
		return convertUnitStr(source_unit, target_unit);
	}
	/**
	 * Added by F.X. Pineau
	 * @param source_unit
	...
	throws ArithmeticException {
		...
		if (target_unit.mksa == source_unit.mksa) {
			if ((target_unit.mksa&_log) == 0) target_unit.value = f*source_unit.value + (source_unit.offset-target_unit.offset)/target_unit.factor;
			else {	// Convert to log scale: log(km) --> log(m)+log(k)
				if ((source_unit.offset != 0) || (target_unit.offset != 0)) throw new ArithmeticException("****Unit: can't convert non-standard unit " + source_unit.symbol + " into " + target_unit.symbol);
				f = AstroMath.log(f);
				if ((target_unit.mksa&_mag) != 0) f = -2.5*f;
		...
		/* The target is a log. Check whether the source is also in log scale */
		if ((source_unit.mksa&_log) != 0) {	// Convert from log to log
			target_unit.value = (source_unit.mksa&_mag) != 0 ? -0.4*source_unit.value :source_unit.value;
			target_unit.value += AstroMath.log(f);  // Target assumed to be log
			if ((target_unit.mksa&_mag) != 0) target_unit.value *= -2.5;
			return ;
		}
		/* The target is a log, the source is in a linear scale. */
		ls = (target_unit.mksa&_mag) != 0 ? -2.5 : 1.;
	 ...
	 * @throws ArithmeticException
	 */
	public String convertFromStr(Unit source_unit) throws ArithmeticException {
		return convertStr(source_unit, this);
	}
	...
	public final void mult (Unit unit) throws ArithmeticException {
		...
		if (((mksa|unit.mksa)&_log) != 0) {
			if ((mksa == _NOUNIT) && (factor == 1.)) ;
			else if ((unit.mksa == _NOUNIT) && (unit.factor == 1.)) ;
		...
		if ((offset!=0) || (unit.offset!=0)) {
			if (mksa == _NOUNIT) offset = unit.offset;
			else if (unit.mksa == _NOUNIT) ;
			else offset = 0;
		}
		v *= unit.value; 
		r *= unit.factor;
		u += unit.mksa; u -= _NOUNIT;
		...
		if ((symbol != null) && (unit.symbol != null)) {
			if ((unit.mksa == _NOUNIT) && (unit.factor == 1)) return;	// No unit ...
			if ((     mksa == _NOUNIT) && (     factor == 1)) symbol = unit.symbol;
