...
public class Image2D extends Product {
	...
	/** * @version $Id: Image2D.java 915 2014-01-29 16:59:00Z laurent.mistahl $
	 ...
	 * @param fileName
	 */
	public Image2D(File file, ProductMapping mapping){		
		super(file, mapping);
		if( mapping != null )
	...
	protected void setWcsFields() throws Exception {
		...
		try{
			wcs.setImage2DCoordinate(this.productAttributeHandler);
		 ...
		 */
		PriorityMode pos_prio = this.mapping.getSpaceAxeMapping().getPriority();
		if( this.astroframe != null && (this.ra_attribute == null || this.dec_attribute == null) &&
				(pos_prio == PriorityMode.LAST || pos_prio == PriorityMode.FIRST)) {
	 ...
	 */
	public void initProductFile(ProductMapping mapping) throws IgnoreException{
	...
	public static void main(String[] args)  {
		...
		try {
			img.initProductFile((new ArgsParser(new String[]{"-debug", "-system='Ecliptic'", "-collection=qwerty"})).getProductMapping());
			//			img.setWcsFields();
