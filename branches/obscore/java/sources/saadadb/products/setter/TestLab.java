 ...
 * @version $Id$
 */
public class TestLab extends ColumnSetter implements Cloneable {
	 ...
	 */
	private TestLabWrapper wrapper;

	
	private SaadaInstance sadInstance;
	/**
	...
	/**
	 * A ColumnSingleSetter used for the abstract methods from ColumnSetter and
	 * when mode=ByValue
	 */
	...
	// /**
	// * Boolean indicating if we've override the automatic detection of
	// settingmode
	// */
	...
	enum INIT {
		CONSTANT_VALUE, CONSTANT_EXPRESSION, SINGLE_ATTRIBUTE, MULTI_ATTRIBUTE,
	};
	 ...
	 * @throws Exception
	 */
	public TestLab(String fieldName, String constantValue) throws Exception {
		super();
		if (constantValue == null) {
			IgnoreException.throwNewException(SaadaException.INTERNAL_ERROR,
					"Column setter cannot be set with a null expression");
		}
		// expression=expr;
	...
	/**
	 * Constructor with only one ah. The expression is set with the name of the
	 * attribute handler/ Its value will be taker as setter value
	 * 
	 * @param attr
	 * @throws Exception
	 */
	/*public ColumnExpressionSetter(String fieldName, AttributeHandler attr)
			throws Exception {
		super();
...
*/
	/**
	 * If the AH map is null, the expression is considered as a set of
	 * operations on constant values. It is computed once.
	 * 
	 * @param expression
	 *            arithmetic expression
	 * @param attributes
	 *            map of the AH possibly used by the expression
	 * @throws Exception
	 */
	public TestLab(String fieldName, String expression, SaadaInstance saadaInstance) throws Exception {
		super();
		...
		this.fieldName = fieldName;
		this.sadInstance = saadaInstance;
		this.setExpression(expression);
		this.calculateExpression();
	}

	/**
	 * Constructor with only one ah calculating the expression immediatly, the
	 * setting mode is manually given
	 * 
	 * @param attr
	 * @throws Exception
	 */
/*	public ColumnExpressionSetter(String fieldName, AttributeHandler attr,
			ColumnSetMode mode) throws Exception {
		this(fieldName, attr);
...
*/
	 ...
	 */
	public TestLab(String fieldName) {
		super();
		this.fieldName = fieldName;
		this.settingMode = ColumnSetMode.NOT_SET;
	}
	...
	private void setExpression(String expression) throws Exception {
		...
		if (this.stringFunctionExtractor.isSingleStringExpression()) {
			...
			if (this.useKeywords()) {
				...
				for (Entry<String, StringFunctionDescriptor> e : stringFunctionExtractor.splitedFunctionsMap
						.entrySet()) {
					this.expression = this.expression.replace(e.getKey(),
							DictionaryStringFunction.exec(
									e.getValue().functionName, e.getValue()
											.getUnquotedArguments()));
				}
			...
			if (this.stringFunctionExtractor != null
					&& !this.stringFunctionExtractor.useKeywords()) {
				for (Entry<String, StringFunctionDescriptor> e : stringFunctionExtractor.splitedFunctionsMap
						.entrySet()) {
					this.expression = this.expression.replace(e.getKey(),
							DictionaryStringFunction.exec(
	...
	public void calculateExpression() throws Exception {
		...
		try {
			if (this.settingMode == ColumnSetMode.BY_KEYWORD) {
			...
			} else if (this.settingMode == ColumnSetMode.BY_VALUE
					|| this.settingMode == ColumnSetMode.NOT_SET) {
				return;
				...
				if (!this.singleStringExpression) {
					// AttributeHandlerExtractor ahExtractor = new
					// AttributeHandlerExtractor(expression, attributes);
					// this.exprAttributes = ahExtractor.extractAH();
					// this.expression=ahExtractor.expression;
					if (!this.expression.trim().equals(
							this.lastExpressionEvaluated)) {
						this.lastExpressionEvaluated = this.expression.trim();
						this.wrapper = new TestLabWrapper(this.expression,sadInstance, this.numericFunctionList);
						this.wrapper.evaluate(this.numericFunctionList);
...
//							this.settingMode = ColumnSetMode.BY_VALUE;
	 ...
	 * @throws Exception
	 */
	private void calculateExpression(SaadaInstance saadaInstance)throws Exception {
		try {
			if (this.settingMode == ColumnSetMode.BY_KEYWORD) {
				this.result = this.singleAttributeHandler.getValue();
				...
				if (this.stringFunctionExtractor != null) {
					...
					if (!this.expression.trim().equals(this.lastExpressionEvaluated)) {
						this.lastExpressionEvaluated = this.expression.trim();
						this.wrapper = new TestLabWrapper(expression,sadInstance, numericFunctionList);
						this.wrapper.evaluate(this.numericFunctionList);
						this.result = wrapper.getStringValue();
	...
	/**
	 * Execute the String functions in the expression and replaced them by their
	 * values
	 * 
	 * @throws Exception
	 * 
	 */
	private void execStringFunction(Map<String, AttributeHandler> attributes)
			throws Exception {
		...
		for (Entry<String, StringFunctionDescriptor> e : this.stringFunctionExtractor.splitedFunctionsMap
				.entrySet()) {
			...
			if (attributes != null && !attributes.isEmpty()) {
				...
				for (int i = 0; i < args.length; i++) {
					...
					if (!(args[i].startsWith("\"") || args[i].startsWith("'"))) {
						...
						if (attributeFound == false)
							IgnoreException
									.throwNewException(
											IgnoreException.WRONG_PARAMETER,
											"Attribute "
													+ args[i]
													+ " referenced by the expression not found in the product attributes");
				...
				/*
				 * If there is no attributes For each argument, we delete the
				 * quotes, if there is no quotes, this is an invalid argument
				 */
				for (int i = 0; i < args.length; i++) {
					if ((args[i].startsWith("\"") || args[i].startsWith("'"))) {
						values[i] = args[i].replaceAll("[\"']", "");
						...
						IgnoreException
								.throwNewException(
										IgnoreException.WRONG_PARAMETER,
										"The String function arguments must be quoted strings or keywords");
				}
			}
	...
	/**
	 * Compute the string function after the the AH used by them have been
	 * extracted. Update the expression with the results of this computation
	 * 
	 * @throws Exception
	 */
	private void execStringFunction() throws Exception {
		...
		for (Entry<String, StringFunctionDescriptor> e : this.stringFunctionExtractor.splitedFunctionsMap
			...
			String[] args = e.getValue().functionArguments;
			String[] values = new String[args.length];
			if (this.stringFunctionArgumentsList != null
					&& !this.stringFunctionArgumentsList.isEmpty()) {
				// We replace the AH name by their values for each argument
				for (int i = 0; i < args.length; i++) {
					attributeFound = false;
					args[i] = args[i].trim();
	 ...
	 * @param attributes
	 *            the AttributHandlers which can be in the expression
	 * @throws Exception
	 */
	// public void calculateExpression(String expr,Map<String,AttributeHandler>
	...
	// {
	// /*
	// * We build the list of AH present in the expression and we format their
	...
	// if(lastExpressionEvaluated!=null &&
	// expression.trim().equals(lastExpressionEvaluated.trim()))
	// {
	...
	// /*
	// * If an exception is risen in the Wrapper (Wrong value in ah, missins
	// value), the result is set to "NULL"
	// */
	// try {
	// if(isNewExpression)
	// {
	// wrapper = new ExpressionWrapper(expression,
	// exprAttributes,numericFunctionList);
	// lastExpressionEvaluated=expression;
	// }
	// else
	// {
	// //If we did evaluate the same expression, we have no need to rebuild the
	// expressionBuilder in the ExpressionWrapper
	...
	/**
	 * Add all numeric function to the ArrayList and treat the special case of
	...
	private void extractNumericFunctions() throws Exception {
		...
		this.expression = numericFunctionExtractor
				.treatConvertFunction(exprAttributes);
	}

	// /**
	// * Execute the String functions in the expression and replaced them by
	// their values
	// * @throws Exception
	// *
	// */
	// private void checkAndExecStringFunction(Map<String,AttributeHandler>
	// attributes) throws Exception
	// {
	// StringFunctionExtractor extractor = new
	// StringFunctionExtractor(this.expression);
	...
	// //for each function
	// boolean attributeFound =false;
	// for(Entry<String,StringFunctionDescriptor> e :
	// extractor.splitedFunctionsMap.entrySet())
	// {
	// String[] args = e.getValue().functionArguments;
	// if(attributes!=null && !attributes.isEmpty())
	// {
	...
	// }
	// if(attributeFound==false)
	// IgnoreException.throwNewException(IgnoreException.WRONG_PARAMETER,
	// "Attribute " + args[i] +
	// " referenced by the expression not found in the product attributes");
	//
	// }
	...
	// * For each argument, we delete the quotes, if there is no quotes, this is
	// an invalid argument
	// */
	// for(int i=0;i<args.length;i++)
	// {
	...
	// }
	// //We execute the String function
	// expression=expression.replace(e.getKey(),
	// DictionaryStringFunction.exec(e.getValue().functionName, args));
	// }
	// }
	// }
	//
	public String toString() {
		...
		retour += "] " + this.getSettingMode() + " " + this.message;
	...
	/*
	 * For each "SetByXXX" method, we give the value to "result" and we set the
	 * mode. If we don't, the old methods will not be able to get the correct
	 * values or will believe the columnsetter is not set (non-Javadoc)
	 * 
	 * @see saadadb.products.setter.ColumnSetter#setByValue(java.lang.String,
	 * boolean)
	 */
	@Override
	...
	public void setByKeyword(boolean fromMapping) {
		this.settingMode = ColumnSetMode.BY_KEYWORD;
		this.completeMessage("keyword <"
				+ this.singleAttributeHandler.getNameorg() + ">");
		if (fromMapping) {
			this.completeMessage("user mapping");
		}
	}
	...
	public void setByWCS(String value, boolean fromMapping) {
		...
		this.result = this.expression = value;
		this.completeMessage("WCS value <"
				+ this.singleAttributeHandler.getValue()
				+ this.singleAttributeHandler.getUnit() + ">");
		if (fromMapping) {
			this.completeMessage("user mapping");
		}
	}
	...
	public void setByPixels(String value, boolean fromMapping) {
		...
		this.result = this.expression = value;
		this.completeMessage("pixel value <"
				+ this.singleAttributeHandler.getValue()
				+ this.singleAttributeHandler.getUnit() + ">");
		if (fromMapping) {
			this.completeMessage("user mapping");
	...
	public void setByTableColumn(String value, boolean fromMapping) {
		...
		this.result = this.expression = value;
		this.completeMessage("content of the column <"
				+ this.singleAttributeHandler.getValue()
				+ this.singleAttributeHandler.getUnit() + ">");
		if (fromMapping) {
			this.completeMessage("user mapping");
	...
	public void setValue(double value, String unit) {
		this.storedValue = this.result = this.expression = String
				.valueOf(value);
		;
		this.setUnit(unit);
	...
	// //Tests
	//
	// Map<String,AttributeHandler> mapTest = new
	// TreeMap<String,AttributeHandler>();
...
//
//		ah5.setNameorg("EMINTEST");
//		ah5.setNameattr("_emintest");
//		ah5.setValue("1");
//		
...
//		/*
//		 * CONSTANT_VALUE, CONSTANT_EXPRESSION, SINGLE_ATTRIBUTE,
//		 * MULTI_ATTRIBUTE,
//		 */
//
//		System.out
//				.println("----------- case CONSTANT_VALUE -------------------");
...
//
//		System.out
//				.println("----------- case CONSTANT_EXPRESSION -------------------");
...
//				"2*12",
//				"2*12 + EMINTEST" ,
//				"2*12 + MJD(_tmax)", "log(2*12) + MJD(_tmax)",
//				"log(EMIN) + MJD(_tmax)", "log(EMIN) + MJD('11/22/1858')",
//				"MJD(TMAX) - MJD(_tmin)", "MJD(TMAX) - MJD('11/22/1858')",
//				"MJD('11/22/1858')", "MJD('11/20/1858') - MJD('11/22/1858')", }) {
...
//			if (ces.notSet()) {
//				System.out.println(mapTest.get("EMINTEST"));
//				break;
//			}
//			ces.calculateExpression(mapTest);
//			System.out.println(" recomputed: " + ces);
//			if (ces.notSet())
//				break;
//		}
//	}
