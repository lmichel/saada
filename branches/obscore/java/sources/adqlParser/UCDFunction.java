...
public class UCDFunction extends UserFunction {
	...
	public UCDFunction(ADQLOperand[] params) throws ParseException {
		...
		if (params == null || params.length < 2 || params.length > 3)
			throw new ParseException("The function UCD must have two or three parameters which corresponds to a table name/alias, a valid ucd and optionally the result unit !");
		...
		if (params[0] instanceof ADQLConstantValue){
			...
			if (tableOp.getType() == ADQLType.STRING){
				...
				if (tableName == null || tableName.trim().length() <= 0)
					throw new ParseException("The first parameter of the UCD function can't be empty. It must be the name of a selected table !");
			}else
				throw new ParseException("\""+tableOp+"\" is not a string and so it is not a valid table name !");
		}else if (params[0] instanceof ADQLColumn)
			tableName = ((ADQLColumn)params[0]).getColumn();
		else
			throw new ParseException("The first parameter of the UCD function must be the name of a selected table !");
		...
		if (params[1] instanceof ADQLConstantValue){
			...
			if (ucdOp.getType() == ADQLType.STRING){
				if (ucdOp.getValue().matches(RegExp.UCD))
					ucd = ucdOp.getValue();
				else
					throw new ParseException("\""+ucdOp.getValue()+"\" is not a valid UCD (for instance: \"em.wl\" is a valid UCD)");
			}else
				throw new ParseException("\""+ucdOp+"\" is not a string and so it is not a valid UCD !");
		}else
			throw new ParseException("The given ucd in the function UCD must be a string. So \""+params[1]+"\" is not valid !");
		...
		if (params.length == 3){
			if (params[2] instanceof ADQLConstantValue){
				...
				if (unitOp.getType() == ADQLType.STRING){
					...
					throw new ParseException("\""+unitOp+"\" is not a string and so it is not a valid unit !");
			}else
				throw new ParseException("The given unit in the function UCD must be a string. So \""+params[2]+"\" is not valid !");
		}
	}
...
//		if (neg)
//			throw new ParseException("The function UCD does not support the unary operator - !");
	}
	...
	public String primaryToSQL() throws ParseException {
		if (replacementColumn != null){
			...
			if (!unit.equalsIgnoreCase(DEFAULT_UNIT)){
				try {
					if (replacementColumn.getMeta().getUcd() == null || replacementColumn.getMeta().getUcd().trim().length() == 0)
						throw new ParseException("Incompatible units: the column \""+replacementColumn.getColumn()+"\" has no unit but the function UCD forces a unit conversion into \""+unit+"\" !");
					else
						sql = UnitHandler.getConvFunction(unit, replacementColumn.getMeta().getUnit(), sql);
			...
			if (alias != null && alias.trim().length() > 0)
				sql += " AS \""+alias+"\"";
			else if (printUcdAsAlias)
				sql += " AS \""+ucd+(unit.equalsIgnoreCase(DEFAULT_UNIT)?"":(" ["+unit+"]"))+"\"";
			return sql;
		}else
			return super.primaryToSQL();
	}
	...
	public ADQLConstraint primaryGetCopy() throws ParseException {
		...
		if (replacementColumn != null)
			copy.setColumn((ADQLColumnAndMeta)replacementColumn.getCopy());
		copy.setAlias(alias);
	...
	public Vector<ADQLObject> primaryGetAll(SearchHandler searchCondition) {
		...
		if (replacementColumn != null){
			if (searchCondition.match(replacementColumn))
				vMatched.add(replacementColumn);
			else{
	...
	public ADQLObject primaryGetFirst(SearchHandler searchCondition) {
		...
		if (replacementColumn != null){
			if (searchCondition.match(replacementColumn))
				matchedObj = replacementColumn;
			else
				matchedObj = replacementColumn.getFirst(searchCondition);
		}
