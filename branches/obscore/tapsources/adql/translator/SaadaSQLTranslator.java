...
//				appendFullDBName(colName, dbCol.getTable());//.append('.');
	...
	public String translate(Comparison comp) throws TranslationException {
		String tmp  = translate(comp.getLeftOperand()) + " " + comp.getOperator().toADQL() + " "
				+ translate(comp.getRightOperand());
		System.out.println("COMPARISON "+tmp);
...
protected boolean pointContainsRAandDEC(PointFunction point) {
	...
	public String translate(DistanceFunction fct) throws TranslationException {
		if (fct == null)
			return null;
		// Original from saada project
		// PointFunction p1 = fct.getP1(), p2 = fct.getP2();
		// return
		// "distancedegree("+p1.getCoord1().toSQL(null)+", "+p1.getCoord2().toSQL(null)+", "+p2.getCoord1().toSQL(null)+", "+p2.getCoord2().toSQL(null)+")";
		...
		if(!pointContainsRAandDEC(p1) ^ !pointContainsRAandDEC(p2)) {
			throw new TranslationException("Either of the point should contains 's_ra' and 's_dec' parameters");
		}
		String result = "distancedegree(" + translate(p1.getCoord1()) + ", " + translate(p1.getCoord2() )+ ", "
	...
	public String translate(ContainsFunction fct) throws TranslationException {
		...
		if (fct.getLeftParam().getName().equalsIgnoreCase("point")) {
			...
			if(!pointContainsRAandDEC(p)) {
				throw new TranslationException("The parameter POINT must contains 's_ra' and 's_dec");	
			}
