...
public class MetaCollection extends MetaObject {
	...
	public void update(ResultSet rs) throws SQLException {
		...
		if( name.startsWith(this.name) == false ) {
			Messenger.printMsg(Messenger.ERROR, "Fatal error when loading meta collection: <" 
	...
	public void update(ResultSet rs, int cat) throws FatalException  {
		try {
			...
			switch(cat) {
			case Category.CUBE:     lah = MetaCollection.attribute_handlers_cube; break;
			case Category.ENTRY:    lah = MetaCollection.attribute_handlers_entry; break;
			case Category.TABLE:    lah = MetaCollection.attribute_handlers_table; break;
			case Category.IMAGE:    lah = MetaCollection.attribute_handlers_image; break;
			case Category.SPECTRUM: lah = MetaCollection.attribute_handlers_spectrum; break;
			case Category.MISC:     lah = MetaCollection.attribute_handlers_misc; break;
			case Category.FLATFILE: lah = MetaCollection.attribute_handlers_flatfile; break;
			default: FatalException.throwNewException(SaadaException.WRONG_PARAMETER, "Internal error: category<" + cat + "> unknown");
			}
		...
		try {
			...
			for( int cat=1 ; cat<Category.NB_CAT ; cat++ ) {
				...
				ResultSet rs = squery.run("SELECT mc.name_attr, mc.ass_error, mc2.name_attr "
						+ " FROM saada_metacoll_" + str_cat + " mc, saada_metacoll_" + str_cat + " mc2 "
	...
	public static final boolean attributeExistIn(String attrName,int category){
		switch(category){
		case Category.CUBE:    return MetaCollection.getAttribute_handlers_cube().containsKey(attrName);
		case Category.ENTRY :  return MetaCollection.getAttribute_handlers_entry().containsKey(attrName);
		case Category.FLATFILE:return MetaCollection.getAttribute_handlers_flatfile().containsKey(attrName);
		case Category.IMAGE:   return MetaCollection.getAttribute_handlers_image().containsKey(attrName);
		case Category.MISC:    return MetaCollection.getAttribute_handlers_misc().containsKey(attrName);
		//case Category.NB_CAT:return this.getAttribute_handlers_nb_cat().containsKey(attrName);
		//case Category.ROOT_PRODUCT:return this.getAttribute_handlers_root_product().containsKey(attrName);
		case Category.SPECTRUM:return MetaCollection.getAttribute_handlers_spectrum().containsKey(attrName);
		case Category.TABLE:   return MetaCollection.getAttribute_handlers_table().containsKey(attrName);
		}
		return false;
	}
	...
	public static final Set<String> getAttribute_handlers_names(int category){
		switch(category){
		case Category.CUBE:    return MetaCollection.getAttribute_handlers_cube().keySet();
		case Category.ENTRY :  return MetaCollection.getAttribute_handlers_entry().keySet();
		case Category.FLATFILE:return MetaCollection.getAttribute_handlers_flatfile().keySet();
		case Category.IMAGE:   return MetaCollection.getAttribute_handlers_image().keySet();
		case Category.MISC:    return MetaCollection.getAttribute_handlers_misc().keySet();
		//case Category.NB_CAT:return this.getAttribute_handlers_nb_cat().keySet();
		//case Category.ROOT_PRODUCT:return this.getAttribute_handlers_root_product().keySet();
		case Category.SPECTRUM:return MetaCollection.getAttribute_handlers_spectrum().keySet();
		case Category.TABLE:   return MetaCollection.getAttribute_handlers_table().keySet();
		}
		return null;
	}
	...
	public boolean implementsDM(VOResource vor, int category) throws InstantiationException, IllegalAccessException, ClassNotFoundException {
		for( String cls: Database.getCachemeta().getClassesOfCollection(this.name, category)) {
			...
			/*
			 * Must be replaced with a meta data access when DM mappings will be stored there.
			 */
			try {
