...
public class ColumnExpressionSetter extends ColumnSetter implements Cloneable {
	 ...
	 * @throws Exception
	 */
	public ColumnExpressionSetter(String fieldName, String expression,
			Map<String, AttributeHandler> attributes) throws Exception {
		super();
		if (attributes == null) {
			...
			if (this.settingMode == ColumnSetMode.BY_KEYWORD) {
				...
				if (!this.singleStringExpression) {
					...
					if (!this.expression.trim().equals(
						...
						this.wrapper = new ExpressionWrapper(this.expression,
								this.exprAttributes, this.numericFunctionList);
						this.wrapper.evaluate(this.exprAttributes,
								this.numericFunctionList);
						this.result = wrapper.getStringValue();
						if (this.exprAttributes.size() == 0
								&& !(this.stringFunctionExtractor != null && this.stringFunctionExtractor
										.useKeywords())) {
							this.settingMode = ColumnSetMode.BY_VALUE;
						}
					} else {
	 ...
	 * @throws Exception
	 */
	private void calculateExpression(Map<String, AttributeHandler> attributes)
			throws Exception {
		try {
			if (this.settingMode == ColumnSetMode.BY_KEYWORD) {
				this.result = this.singleAttributeHandler.getValue();
				...
				if (this.stringFunctionExtractor != null) {
					...
					if (!this.expression.trim().equals(
							this.lastExpressionEvaluated)) {
						this.lastExpressionEvaluated = this.expression.trim();
						this.wrapper = new ExpressionWrapper(expression,
...
//				"MJD('11/22/1858')", "MJD('11/20/1858') - MJD('11/22/1858')", 
			...
			System.out.println(ces.getExpression());
			//System.out.println("@ "+ces.getExpressionResult());
			if (ces.notSet()) {
