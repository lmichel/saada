...
while(it.hasNext()) {
	...
	private TAPTable fetchTableMeta(final String tableName, final String userId, final FieldSet fields) throws TAPException{
		...
		for(int j=0 ; j<fields.getItemCount(); j++){
			...
			if(field.getName().contains("."))
				throw new TAPException("Column's name '"+field.getName()+"' must not contains any '.'");
			...
			if (field.getArraySize() == null || field.getArraySize().trim().isEmpty())
				...
				try{
					...
					Messenger.locateCode("Code ADDED TO TAP library");
					//TODO Code ADDED TO TAP library
					arraysize=TAPTypes.STAR_SIZE;
				}
			}
