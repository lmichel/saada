...
//		.attr("src", "images/ivoa.jpg");
	...
	var openModalWithClose = function() {
		...
		Modalinfo.dataPanel("Samp Info"
				, getElement('').html()
				, function() {
					if( that.modalOpen ) WebSamp_mVc.fireCloseModal();
					});
	};
	/*************************************************************************************************
	...
	var fireRegisterToHubAttempt = function() {
		if (waitForHub) {
			...
			waitForHub = false;
			fireRegisterToHub();
			//setTimeout("WebSamp_mVc.fireRegisterToHubAttempt();", 5000);
		} else {
			waitForHub = false;
		}
	};
	...
	var showHubError = function(message) {
		...
		openModalWithClose();
		setTimeout(WebSamp_mVc.fireRegisterToHubAttempt, 3000);
	...
	var fireLaunchHub = function(name, url) {
		...
		setIvoaIcon();
		//setTimeout("WebSamp_mVc.fireRegisterToHubAttempt();", 1000);
	};
