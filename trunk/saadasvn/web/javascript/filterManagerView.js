jQuery.extend({
	...
	FilterManagerView : function() {
		...
		this.fireNativeEvent = function(uidraggable){
			$.each(listeners, function(i){
				listeners[i].controlNativeEvent(uidraggable);
			});
		}
		...
		this.showFilterManager = function(jsdata, speFields, attributesHandlers, relations) {
			if (jsdata.errormsg != null) {
				alert("FATAL ERROR: "+ jsdata.errormsg);
				return;
			}


			var obj = jQuery.parseJSON(jsdata);
			var content;
			
			if ((obj.category != null) && (obj.collection != null)) {
				content = "<h2>Editing filter of the category " + obj.category ;
				if (obj.collection != "Any-Collection") {
					content += " from the collection " + obj.collection + "</h2>";
				} else {
...
//			content += "<input type=button OnClick=\"filterManagerView.fireSaveFilter();\" title=\"Save this display filter for the current collection/category\" value=\"Save Filter\">&nbsp;&nbsp;";
			...
			that.initFieldsList(speFields, attributesHandlers, relations);
			that.fireInitExistingFields();
		}
		
		this.initFieldsList = function(speFields, attributesHandlers, relations){
			...
			for( i in attributesHandlers ) {
				table += "<li class=\"ui-state-default\"><span class=name>" 
					+ attributesHandlers[i].nameorg 
					+ " (" + attributesHandlers[i].type 
					+ ")</span><span class=hidden>" + attributesHandlers[i].nameattr + "</span></li>";
			}
		...
		this.updateNatives = function (nname, nnameorg) {
