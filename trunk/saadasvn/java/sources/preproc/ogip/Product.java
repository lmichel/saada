...
public class Product extends File {
	...
	public Product(String fileName){
		...
		for(int i = 0; i < Product.tabIgnored.length; i++)
			this.vIgnored_att.add(Product.tabIgnored[i]);
	...
	public void getFirstHDUAttributes(){
		...
		while( it.hasNext() ){
			...
			if( !this.vIgnored_att.contains(key) && key != null ){
				...
				if( value != null && !hCard.isStringValue() ){ //if the value is not a string then we assign the correct type
	...
	public void getParametersData(){
		...
		float[] values = null;
			//Vector of array wich contains each values of each parameters
		Vector<float[]> vValues = new Vector<float[]>();	
			//vector wich contains all the combinations of parameters
		...
		for(int i=1; i<vValues.size(); i++){
			...
			for( int j=0; j<this.vCombi.size(); j++){
				...
				for(int k=0; k<tabCourant.length; k++){
					...
					for(int m=0; m<vCombiCourant.size(); m++)
						vCombiCourantTmp.add(vCombiCourant.get(m));
			...
			this.vCombi = new Vector<Vector<Float>>();
			for(int m=0; m<vCombiTmp.size(); m++)
				this.vCombi.add( vCombiTmp.get(m) );	
	...
	public void getUnitAndName(){
		...
		while( it.hasNext() ){
			...
			if( !this.vIgnored_att.contains(key) && key != null ){
				...
				if( key.equals("TUNIT2") )
					this.unit = hCard.getValue();
				
				if( key.equals("TTYPE2") )
					this.fieldName = hCard.getValue();
	...
	public void setVMapAttribute(
			Vector<HashMap<String, AttributeHandler>> mapAttribute) {
