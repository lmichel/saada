...
public class SQLQuery {
	...
	protected boolean isSQLQueryClosed;
	...
	// // _stmts = SaadaDBConnector.getConnector(null,
	// false).getConnection().createStatement(Database.getWrapper().getDefaultScrollMode(),Database.getWrapper().getDefaultConcurentMode());
	// connector = SaadaDBConnector.getConnector(null,
	// false).getJDBCConnection();
	// }
	// else {
	// // _stmts =
	// Database.get_connection().createStatement(Database.getWrapper().getDefaultScrollMode(),Database.getWrapper().getDefaultConcurentMode());
	// connector = Database.get_connection();
	// }
	// if (Messenger.debug_mode)
	// Messenger.printMsg(Messenger.DEBUG, "autocommit switched on " +
	// connector.isReadOnly());
	// connector.setAutoCommit(true);
	// _stmts =
	// connector.createStatement(Database.getWrapper().getDefaultScrollMode(),Database.getWrapper().getDefaultConcurentMode());
	// } catch(Exception e) {
	// QueryException.throwNewException(SaadaException.DB_ERROR, e);
	// }
	//
	...
	public ResultSet run(String sql, String[] tables_to_lock)
			throws QueryException {
		if (tables_to_lock != null) {
			try {
				SQLTable.lockTables(null, tables_to_lock);
				/*
				 * Abort exception is downraded in QueryException because we are
				 * here in read mode.
				 */
	...
	public ResultSet run() throws QueryException {
		try {
			...
			try {
				...
				Messenger
						.printMsg(Messenger.WARNING,
								"Takea new connection  due to the following error "
										+ e);
				Messenger.printStackTrace(e);
				_stmts.close();
				...
				// "Reopen DB connection due to " + e);
				// Database.get_connection().close();
				// SaadaDBConnector sc = SaadaDBConnector.getConnector(null,
				// false);
				// sc.reconnect();
				// _stmts =
				// sc.getJDBCConnection().createStatement(Database.getWrapper().getDefaultScrollMode(),Database.getWrapper().getDefaultConcurentMode());
				resultset = _stmts.executeQuery(query);
			}
			if (Messenger.debug_mode)
				Messenger
	...
	public void close() throws QueryException {
		...
		//If the query has NOT already been closed, close it
	...
	/**
	 * The same as runQueryUpdateSQL but without throwing exceptions. That
	 * allows to achieve operations on corrupted DBs. (e.g. collection removal)
	 * if an operation fails, the process can continue
	 * 
	 * @param sql
	 * @param table_to_lock
	 * @return
	 ...
	 */
	public static void forceUpdateQueryXX(String sql, String table_to_lock)
			throws FatalException {
		try {
			if (table_to_lock != null && table_to_lock.length() > 0) {
				SQLTable.addQueryToTransaction(Database.getWrapper().lockTable(
						table_to_lock));
			}
		} catch (Exception e) {
			Messenger.printMsg(Messenger.ERROR,
					"Query: " + sql + ": " + e.toString());
		}
