				...
				value : function() {
					...
					var html = '<div>'
						...
						+ ((this.frames != null) ? '      <select id="'
								...
								+ this.uploadId
								+ '" value="Upload Position List" title="Not implemented yet (wait for next release)"/>'
								+ '</div>'
								+ '       <div>'
								+ '            <span class=help id="uploadresult"></span>'
								+ '       </div>' + ' </div>';
							...
							.uploadForm(
									...
									function(returnedPath) {
										...
										if (returnedPath.retour.nameVot != undefined
											...
											console.log(parentId.split("_")[0]+'_delconstlist');
...
ConstQEditor_mVc.prototype = {
		...
		fireOrderBy : function(nameattr, desc){
			if( nameattr != 'OrderBy' ) {
				this.orderByView.setOrderBy(nameattr);
				if( desc == false ) {
					this.orderByView.setAsc()
				} else {
					this.orderByView.setDesc()
...
MetadataSource = function() {
	...
	var getTableAtt =  function(dataTreePath /* instance of DataTreePath */, handler){
		...
		if(  cache[key] == undefined ) {
			...
			Out.info("get cached stored node  " + JSON.stringify(dataTreePath) + " length = " + cache[key].hamap.length);
	...
	var getTableAttASync =  function(dataTreePath /* instance of DataTreePath */, handler){
		...
		if( !cache || !cache[dataTreePath.key] ) {
			...
			Out.info("get cached stored node  (async) " + dataTreePath.nodekey + " length = " + cache[dataTreePath.nodekey].hamap.length);
